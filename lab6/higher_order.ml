...
let fourth = twice square let pipeline x f = f x let (|>) = pipeline
...
let sum_cube_odd n = List.fold_left (+) 0 (List.map cube (List.filter (fun x -> x mod 2 <> 0) (0 -- n)))

let sum_cube_odd_pipe n = (0 -- n) |> List.filter is_odd |> List.map cube |> List.fold_left (+) 0
