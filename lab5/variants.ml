...
let rec min_max_helper fx tree =
  match tree with
  | Leaf -> failwith "empty tree"
  | Node ((k, v), Leaf, Leaf) -> k
  | Node ((k, v), Node ((a, b), c, d), Leaf) -> fx k (min_max_helper fx (Node ((a, b), c, d)))
  | Node ((k, v),Leaf, Node ((a, b), c, d)) -> fx k (min_max_helper fx (Node ((a, b), c, d)))
  | Node ((k, v), Node ((e, f), g, h), Node ((a, b), c, d)) -> fx (min_max_helper fx (Node ((e, f), g, h))) (fx k (min_max_helper fx (Node ((a, b), c, d))))

let minimium tree = min_max_helper min tree
let maximium tree = min_max_helper max tree
